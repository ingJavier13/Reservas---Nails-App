...
export default function AdminPanel({ user, token }) {
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-12 animate-fade-in">
      ...
      {(user?.role === 'ADMIN' || user?.role === 'TRABAJADOR') && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl relative overflow-hidden group hover:scale-[1.02] transition-transform">
             ...
             <h3 className="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-2">Citas para Hoy</h3>
             <div className="flex items-end gap-3">
               <span className="text-4xl font-black text-amber-400">{citasHoy.length}</span>
               <span className="text-xs text-slate-400 mb-1 pb-1">programadas</span>
             </div>
             {proximasHoy.length > 0 ? (
               <div className="mt-4 pt-4 border-t border-slate-800">
                 <p className="text-[10px] uppercase tracking-widest text-slate-500 mb-2">Próxima cita:</p>
                 <p className="text-sm font-medium text-slate-200">
                   {new Date(proximasHoy[0].date).toLocaleTimeString('es-MX', {hour: '2-digit', minute:'2-digit'})} - <span className="font-bold text-amber-400">{proximasHoy[0].user?.name}</span>
                 </p>
               </div>
             ) : (
               <div className="mt-4 pt-4 border-t border-slate-800">
                 <p className="text-[10px] uppercase tracking-widest text-slate-600">Día completamente libre</p>
               </div>
             )}
          </div>
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 mb-6 flex flex-wrap gap-4 items-center animate-fade-in">
         <input 
           type="text" 
           placeholder={user?.role === 'ADMIN' ? "Buscar cliente o servicio..." : "Buscar servicio..."}
           className="flex-1 min-w-[200px] p-3 bg-slate-50 border border-slate-100 rounded-2xl focus:border-slate-900 outline-none transition text-xs font-medium text-slate-900 shadow-inner" 
           value={searchTerm} 
           onChange={(e) => setSearchTerm(e.target.value)}
         />
         <input 
           type="date"
           className="p-3 bg-slate-50 border border-slate-100 rounded-2xl focus:border-slate-900 outline-none transition text-xs font-medium text-slate-500 shadow-inner uppercase tracking-wider" 
           value={filterDate}
           onChange={(e) => setFilterDate(e.target.value)}
         />
         <select 
           className="p-3 bg-slate-50 border border-slate-100 rounded-2xl focus:border-slate-900 outline-none transition text-xs font-bold uppercase tracking-widest text-slate-600 shadow-inner" 
           value={filterStatus}
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        {filteredAppointments.length === 0 ? (
          <div className="p-8 text-center text-sm font-bold uppercase tracking-widest text-slate-400">
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredAppointments.map(cita => (
                  <tr key={cita.id} className="hover:bg-slate-50 transition">
                    ...
                    {user?.role === 'ADMIN' && (
                      <td className="p-4">
                        <span className="block font-medium text-slate-700">{cita.user?.name || 'Cliente'}</span>
                        {cita.user?.phone && (
                          <a href={`https://wa.me/52${cita.user.phone}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] text-teal-700 hover:text-teal-900 font-bold tracking-widest uppercase mt-1 transition-colors">
                            <MdPhone size={12} /> {cita.user.phone}
                          </a>
                        )}
                      </td>
                    )}
                    <td className="p-4">
                      <span className="font-bold text-teal-800">{cita.service?.name}</span>
                    </td>
                      ...
                      <div className="flex justify-center flex-wrap gap-2">
                        {user?.role === 'ADMIN' && (!cita.status || cita.status === 'PENDIENTE') && (
                          <button 
                            ...
                            className="text-slate-900 border border-slate-900 hover:bg-slate-900 hover:text-white font-bold px-4 py-1.5 rounded-full text-[10px] uppercase tracking-widest transition-all duration-300"
                          >
                            Aceptar
                          </button>
                        )}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-[2rem] p-8 max-w-md w-full shadow-2xl relative">
            ...
            <form onSubmit={confirmarCancelacion}>
              <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Motivo de Cancelación</label>
              <textarea 
                required 
                className="w-full p-4 bg-slate-50 border border-slate-200 focus:border-slate-900 outline-none transition rounded-2xl font-medium text-slate-900 resize-none shadow-sm" 
                rows="3"
              ...
              <div className="flex gap-3 mt-8">
                <button 
                  ...
                  className="flex-1 py-4 font-bold text-slate-500 border border-slate-200 hover:bg-slate-50 hover:text-slate-900 rounded-full transition-all uppercase tracking-widest text-[10px]"
                >
                  Regresar
                </button>
                <button 
